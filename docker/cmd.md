...
2. 这时候启动如果仍然退出，是因为启动脚本不是一个前台持续的进程，当这个脚本执行完毕后，容器自然就关闭了。

3. 如果启动时使用了不是持续进行的脚本，一种简单的方式是使用tail -f 持续监听，则容器就不会关闭了。
...
./start-hbase.sh
echo "Service hbase start..."

tail -f /usr/share/hbase/logs/*
